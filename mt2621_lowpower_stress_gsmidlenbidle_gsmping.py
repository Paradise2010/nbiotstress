...
def DMDSGSMIDLENBIDLEGSMPING_setting(ser,event_handler):


    logger.debug("start dmds gsm idle nb iot idle ping setting!")
    ser.settimeout(2)
    executeAtCommand(ser,"AT+EUSO=1","OK")
    executeAtCommand(ser,"AT*MNVMQ=\"1.0.0\"","OK")
    #AT*MNVMQ="1.0.0"
    time.sleep(1)
    #AT*MNVMW=1,"NVDM_MODEM_CFG","CELL_QUALITY_RSRP_THRESHOLD",0,1,"78"
    executeAtCommand(ser,"AT*MNVMW=1,\"NVDM_MODEM_CFG\",\"CELL_QUALITY_RSRP_THRESHOLD\",0,1,\"4B\"","OK")
    time.sleep(2)
    ser.close()
    logger.info("close port")
    event_handler.resetboard()
    time.sleep(10)
    if ser.isopen == False:
        logger.info("open port")
        ser.open()
    logger.info(str(ser.isopen))
    ser.open()
    ser.start()
    time.sleep(1)
    ser.Send_Command("AT\r")
    ser.Send_Command("AT\r")
    executeAtCommand(ser,"AT+EUSO=0","OK")
    executeAtCommand(ser,"AT+EBGTHD=-90","OK")
...
def DMDSGSMIDLENBIDLEGSMPING(ser,i):
    ...
    executeAtCommand(ser,"AT+EGDCONT=0,\"IP\",\"CMNET\"","OK")
    ser.settimeout(40)
    ser.Send_Command('AT+PING=182.150.27.42 -n 4 -w 10000\r')
    ...
    if result == 4:
        ...
        Realtime=time.strftime("%Y%m%d_%H%M%S",time.localtime(time.time()))
        writercsv([loop,Realtime,'GSMIDLENBIDLEGSMPING','pass','ping success!'])
        #return False
    else:
        if result  < 2 :
            #write_record(run_record_file,at_exe_time,'AT+Ping=182.150.27.42',"Pass",command_response)
            Realtime=time.strftime("%Y%m%d_%H%M%S",time.localtime(time.time()))
            writercsv([loop,Realtime,'GSMIDLENBIDLEGSMPING','Fail','ping failed'])
            #return True
...
if __name__ == "__main__":
    ...
    try:
		...
		DMDSGSMIDLENBIDLEGSMPING_setting(ser,event_handler)
		for i in range(1000):
			DMDSGSMIDLENBIDLEGSMPING(ser,i)
