...
def executeAtCommand(serinstance,atcommand,response):
    ...
    if RSP != "timeout":
        logger.debug(RSP)
        return True
def DMDSGSMIDLENBIDLEPING_setting(ser):
    ...
    executeAtCommand(ser,"AT+MSSTANDBY=1","CPIN: READY")
    ser.settimeout(2)
    executeAtCommand(ser,"AT","OK")
    executeAtCommand(ser,"AT","OK")
    ...
    if i == 60:
        ...
        return False
def DMDSGSMIDLENBIDLEPING(ser,i):
    ...
    if result == 4:
        ...
        Realtime=time.strftime("%Y%m%d_%H%M%S",time.localtime(time.time()))
        writercsv([loop,Realtime,'DMDSGSMIDLENBIDLEPING','Fail','ping success!'])
        #return False
    else:
        if result  < 2 :
            #write_record(run_record_file,at_exe_time,'AT+Ping=182.150.27.42',"Pass",command_response)
            Realtime=time.strftime("%Y%m%d_%H%M%S",time.localtime(time.time()))
            writercsv([loop,Realtime,'DMDSGSMIDLENBIDLEPING','Fail','ping failed'])
            #return True
    time.sleep(1)
...
if __name__ == "__main__":
    ...
    try:
		...
		DMDSGSMIDLENBIDLEPING_setting(ser)
		for i in range(1000):
			DMDSGSMIDLENBIDLEPING(ser,i)
